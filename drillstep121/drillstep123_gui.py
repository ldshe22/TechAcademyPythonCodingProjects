...
def load_gui(self):
    ...
    self.btn_add = tk.Button(self.master,width=30,height=4,text='Browse Source Directory',command=lambda: drillstep123_func.OnButtonClick(self))
    self.btn_add.grid(row=1,column=0,rowspan=1,columnspan=1, padx=(20,0),pady=(100,00),sticky=N+W)
    self.btn_2add = tk.Button(self.master,width=30,height=4,text='Browse Destination Directory',command=lambda: drillstep123_func.OnButtonClick(self))
    ...
    self.txt_2add = tk.Entry(self.master,text='')
    self.txt_2add.grid(row=2,column=2,rowspan=4,columnspan=10,padx=(40,0),ipadx=330,pady=(10,0),ipady=20,sticky=N+W)
