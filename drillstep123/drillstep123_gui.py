...
def load_gui(self):
    ...
    self.btn_add = tk.Button(self.master,width=30, height=4, text='Browse Source Directory', command=lambda: drillstep123_func.onbuttonclicksource(self))
    self.btn_add.grid(row=1, column=0, rowspan=1, columnspan=1, padx=(20, 0), pady=(100, 00), sticky=N+W)
    self.btn_2add = tk.Button(self.master, width=30, height=4, text='Browse Destination Directory', command=lambda: drillstep123_func.onbuttonclickdestination(self))
    self.btn_2add.grid(row=2, column=0, rowspan=1, columnspan=1, padx=(20, 0), pady=(10, 00), sticky=N+W)
    self.btn_ck = tk.Button(self.master, width=30, height=6, text='Check for files...', command=lambda: drillstep123_func.onbuttonclickcheckfiles(self))
